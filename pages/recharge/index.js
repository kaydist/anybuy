...
import { AddToRechargeCart, AddToTotalRechargePrice } from "../../store/actions/quantityChange";
...
function BuyData(type = "Airtime") {
  const dispatch = useDispatch();
  const currentTotalCartPrice = useSelector(state=> state.rechargeCart.totalCartPrice)
  const router = useRouter();
  const [rechargeType, setRechargeType] = useState(type);
  const [inputList, setInputList] = useState([
    { name: "", mobileNumber: "", data: "", amount: "", image: "" },
  ]);
  ...
  const handleReadOnlyChange = (name, value, index) => {
    const list = [...inputList];
    list[index][name] = value;
    setInputList(list);
    if(name === 'amount'){
      setTotalCartPrice(Number(currentTotalCartPrice) + Number(value))
    } return
  };
  ...
  const handleAddClick = (networkName) => {
    ...
    dispatch(AddToTotalRechargePrice(totalCartPrice));
    setInputList([
      ...inputList,
      { name: "", mobileNumber: "", data: "", amount: "", image: "" },
    ]);
      ...
      {inputList.map((form, idx) => {
        return (
          <div id="duplicated_cards_container" key={idx}>
            <div
              className="card my-8 flex flex-col gap-8 lg:max-w-2xl justify-between"
              id="duplicater0"
            >
              {inputList.length > 1 ? (
                <p
                ...
                {({
                  ...
                  >
                    ...
                    <div className="flex w-full flex-wrap gap-1">
                      {rechargeType === "Data" ? (
                        <div className="flex flex-col w-full lg:w-1/2">
                          <label className="font-bold">Enter Data Value</label>
                          <input
                            type="number"
                            name="data"
                            onChange={(e) => {
                              ...
                              handleReadOnlyChange(name, value, idx);
                            }}
                            onBlur={handleBlur}
                            value={values.name}
                            placeholder=""
                    ...
                    <div className="flex flex-row items-start gap-2 w-full lg:w-1/2 ">
                      ...
                      <span className="text-xs">
                        Want to recharge on a new phone or buy different plan on
                        a old phone
                      </span>
                    </div>

                    {
                      ...
                      >
                        Recharge More
                      </p> //find parameter to tie to so it will not show in the first case before validation proceed
                    }
                  </form>
                )}
              </Formik>
